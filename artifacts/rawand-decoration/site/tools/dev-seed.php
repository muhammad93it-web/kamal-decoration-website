 ...
 *  - creates the dev admin account (admin / Rawand@2026) when users is empty
...
if ($n === 0) {
    ...
    $st->execute(['admin', 'ڕەوەند', password_hash('Rawand@2026', PASSWORD_DEFAULT)]);
    ...
    echo "[seed] created admin user (admin / Rawand@2026)\n";
}

/* 2) site_url from Replit env (+ dev base path when served under a prefix) */
$domain = getenv('REPLIT_DEV_DOMAIN') ?: strtok((string)getenv('REPLIT_DOMAINS'), ',');
if ($domain) {
    $bp = rtrim((string)(getenv('BASE_PATH') ?: ''), '/');
    $want = 'https://' . trim($domain) . ($bp !== '' && $bp !== '/' ? $bp : '');
